...
function scene:create( event )
   ...
   local galaxyName="galaxy"..math.random(6)..".png"
   ...
   function btnLevelTap(event)
      ...
      if event.target.enabled==true then
         ...
         elseif event.target.level == 5 then composer.gotoScene( "level" , {effect="fade", time=500, params = {planetNo=2,volcano=false,level=event.target.level} })
         elseif event.target.level == 6 then composer.gotoScene( "level" , {effect="fade", time=500, params = {planetNo=1,volcano=false,level=event.target.level} })
         elseif event.target.level == 7 then composer.gotoScene( "level" , {effect="fade", time=500, params = {planetNo=2,volcano=false,level=event.target.level} })
         elseif event.target.level == 8 then composer.gotoScene( "level" , {effect="fade", time=500, params = {planetNo=3,volcano=false,level=event.target.level} })
...
function scene:show( event )
   ...
   if ( phase == "will" ) then
      ...
      local starsCount=0
      for i=1,12 do 
         if (G.gamedata.levelEnabled[i] and (i==1 or (i>=2 and math.floor(starsCount/(i-1))>=2))) or G.debugFuel then
